...
const ProjectionThreadDbRowSchema = ProjectionThread.mapFields(
  Struct.assign({
    ...
    redo: Schema.NullOr(Schema.fromJsonString(OrchestrationThreadRedoState)),
  }),
);
...
interface ThreadAssemblyRows {
  ...
  readonly messageRows: ReadonlyArray<
    Schema.Schema.Type<typeof ProjectionThreadMessageDbRowSchema>
  >;
  readonly proposedPlanRows: ReadonlyArray<
    Schema.Schema.Type<typeof ProjectionThreadProposedPlanDbRowSchema>
  >;
  readonly activityRows: ReadonlyArray<
    Schema.Schema.Type<typeof ProjectionThreadActivityDbRowSchema>
  >;
  readonly checkpointRows: ReadonlyArray<
    Schema.Schema.Type<typeof ProjectionCheckpointDbRowSchema>
  >;
...
/**
 * Builds fully populated OrchestrationThread values from projection rows.
 *
 * Shared by getSnapshot and getCommandReadModel so both always load the
 * complete thread content (messages, activities, checkpoints, redo). The
 * decider depends on this: branching copies messages and redo validation
 * reads checkpoints, which must survive server restarts.
 */
function assembleThreadsFromRows(rows: ThreadAssemblyRows): {
  ...
  const threads: ReadonlyArray<OrchestrationThread> = rows.threadRows.map((row) => ({
    id: row.threadId,
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
        ...
        Effect.flatMap(
          ([
            ...
            Effect.gen(function* () {
              ...
              const assembled = assembleThreadsFromRows({
                threadRows,
                messageRows,
                proposedPlanRows,
                activityRows,
              ...
              updatedAt =
                assembled.updatedAt !== null ? maxIso(updatedAt, assembled.updatedAt) : updatedAt;
  ...
  // Loads the full thread content, not a shallow shell: the decider needs
  // messages (thread.branch copies them), checkpoints (revert/redo
  // validation), and the redo stash — all of which must survive restarts.
